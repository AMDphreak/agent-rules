...
| `bitwarden-unlock` | bitwarden, bw, BW_SESSION, unlock_bitwarden.ps1, vault unlock, piping session | active | Workstation auth; never print/read session; template in `scripts/` |
| `writing-news-vs-blog` | news vs blog body copy, lede, attribution, anti-slop article prose, ship notes, essays | active | Personal fork; team `writing-news` / `writing-blog` for org work |
